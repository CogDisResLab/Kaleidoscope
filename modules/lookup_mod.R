      ...
      tabPanel(
        ...
        fluidRow(
          ...
          column(
            ...
            div(
              id = ns("dn_btn_div"),
              download_btn_ui(ns("dn_btn")),
              download_btn_ui(ns("dn_btn2"), label = "Download with pvalues")
            )
          ),
          column(width = 12, hr()),
          column(width = 5,
  ...
  moduleServer(id,
               function(input, output, session) {
                 updatePickerInput(session,
                                   "dbs1",
                                   choices = filter(ks_datasets, Group == "SCZ") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs2",
                                   choices = filter(ks_datasets, Group == "MDD") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs3",
                                   choices = filter(ks_datasets, Group == "Dop") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs4",
                                   choices = filter(ks_datasets, Group == "AP") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs5",
                                   choices = filter(ks_datasets, Group == "ATD") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs6",
                                   choices = filter(ks_datasets, Group == "AD") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs7",
                                   choices = filter(ks_datasets, Group == "Ins") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs8",
                                   choices = filter(ks_datasets, Group == "BPD") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs9",
                                   choices = filter(ks_datasets, Group == "Aging") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs10",
                                   choices = filter(ks_datasets, Group == "MCLR") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs11",
                                   choices = filter(ks_datasets, Group == "KID") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs12",
                                   choices = filter(ks_datasets, Group == "MYO") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs13",
                                   choices = filter(ks_datasets, Group == "AST") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs14",
                                   choices = filter(ks_datasets, Group == "PTSD") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs15",
                                   choices = filter(ks_datasets, Group == "Ketosis") %>% pull(DataSet))
                 updatePickerInput(session,
                                   "dbs16",
                                   choices = filter(ks_datasets, Group %in% c("COVID", "Other", "MB")) %>% pull(DataSet))
                 ...
                 observeEvent(genes_ids$btn(), {
                   ...
                   if (!any(all(trimws(genes_raw) == "") |
                            is.null(datasets_selected))) {
                     w$show()
                     ...
                     look_res <<-
                       withProgress(message = "connecting to KS database ...", {
                         list(lookup_df = ks_lookup(genes = genes, datasets = datasets_selected)
                             ...
                             output$plot2 <- renderEcharts4r({
                               ...
                               output$plot3 <- renderPlot({
                                 look_res$lookup_df %>%
                                   ...
                                   ungroup() %>%
                                   ggplot(aes(
                                     x = reorder(HGNC_Symbol, Log2FC, median),
                                     y = Log2FC,
                                     fill = mean_LFC
                                   )) +
                                   ggdist::stat_halfeye(
                               ...
                               output$plot4 <- renderEcharts4r({
                                 look_res$lookup_df %>%
                                   ...
                                   summarise(
                                     ...
                                     Unchnaged = sum(
                                       Log2FC > (input$slider1 * -1) && Log2FC < input$slider1
                                     )
                                   ) %>%
                                   ...
                                   e_pie(
                                     n,
                                     radius = c("40%", "60%"),
                                     itemStyle = list(borderRadius = 10, borderWidth = 2),
                               ...
                               output$heatmap_plot <- plotly::renderPlotly({
                                 ...
                                 labelmatrix <-
                                   matrix(paste0("LFC: ", as.numeric(unlist(mm))),
                                          nrow = nrow(mm),
                                          ncol = ncol(mm))
                                 ...
                                 heatmaply::heatmaply(
                                   matrix_rescale(
                                     mm,
                                     min = (input$hm_slider1 * -1),
                                     max = input$hm_slider1
                                   ),
                                   scale = "none",
